...
class MusicXMLReader
{
    ...
    vector<float> v_duration; // вектор с длительност€ми
    vector<int> part_list; // номер дорожки, используетс€ дл€ выбора
    int divisions = 1; //переменна€ дл€ правильного расчЄта всех длительностей в партии
    ...
    int line = 0; //»спользуетс€ дл€ ключей.2 дл€ знака G (скрипичный ключ), 4 дл€ знака F (басовый ключ) и 3 дл€ знака C (альтовый ключ).
    int bpm = 0; // beat per minute
    int count_measure = 1; //счЄтчик тактов с 1
    int duration = 0; //счЄтчик тактов с 1
    int voice = 0; //голоса
    int step = 0; // шаг дл€ pitch
    int octave = 0; // октава
    int alter = 0; //показывает полутон на ноте, если он есть на ключе
    int chromatic = 0; // количество тонов дл€ перехода от написанного к звучащему тону
    string sign; // символ ключа
    string bpm_unit; //обозначение bmp дл€ величины
