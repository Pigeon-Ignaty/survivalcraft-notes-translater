...
MusicXMLReader::MusicXMLReader(const char* name) {
    ...
    if (doc.Error()) {
        ...
        if (NULL != pRootElement) { // если не пустой
            XMLElement* credit = pRootElement->FirstChildElement("credit"); // спускаемс€ в credit
            while (credit) {
                XMLElement* credit_type_tag = credit->FirstChildElement("credit-type"); // в цикле вытаскиваем тип
                XMLElement* credit_words_tag = credit->FirstChildElement("credit-words"); // в цикле вытаскиваем им€ типа

                if (NULL != credit_type_tag && credit_words_tag != NULL) { // если не пустой
                    cout << credit_type_tag->GetText() << ": " << credit_words_tag->GetText() << "\t"; // вывод автора и названи€
                }
                cout << endl;
                credit = credit->NextSiblingElement("credit"); //след элемент
            }
            cout << "¬ывод следующих партий в музыкальной композиции: " << endl;
            ...
            if (part_list != NULL) {
                    ...
                    score_part != NULL; score_part = score_part->NextSiblingElement("score-part"))
                {
                    ...
                    if (part_name != NULL) {
                        cout << "Ќазвание партии: " << string(part_name->GetText()) << endl;
                    }
                    scorePartCount++;
                }
            }
...
void MusicXMLReader::MusicPartWriter(const char* NamePart) {
 ...
 void MusicXMLReader::OutputToConsole() {
     ...
     cout << endl;
     cout << "Ќоты в v_notes" << endl;
     for (int i = 0; i < v_notes.size(); i++) {
         cout << v_notes[i] << "";
     }
     cout << endl;
     cout << "ќктавы в v_octaves" << endl;

     for (int i = 0; i < v_octaves.size(); i++) {
         cout << v_octaves[i] << "";
     }
     cout << endl;
     cout << "ƒлительности в v_duration" << endl;
 ...
 void MusicXMLReader::show_information_about_composition(vector<int>& duration, int fraction_numerator, int denominator_fraction, int bpm) {
     ...
     cout << "ƒлина минимальной длительности/врем€ дл€ генератора " << round(time_min_duration * 100) / 100 << endl;
     cout << "ћинимальна€ длительность " << max_duration << endl;
